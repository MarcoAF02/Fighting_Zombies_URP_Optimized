...
{
    static string ExcelDataPath = Application.dataPath + "/../Excel_Data";//源Excel文件夹,xlsx格式
    static string BytesDataPath = Application.dataPath + "/Resources/Data_Table";//生成的bytes文件夹
    static string CsClassPath = Application.dataPath + "/Scripts/Data_Table";//生成的c#脚本文件夹
    static string XmlDataPath = ExcelDataPath + "/Temp_Xml_Data";//生成的xml(临时)文件夹..
	...
	// 如果是写 CS 文本（Excel 读到 C# 代码），按行和列读取后直接写入即可
	// 如果是写为二进制文件，读取后先生成一份 XML 临时文件，根据这份 XML 临时文件生成二进制文件。
